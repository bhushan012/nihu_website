...
class RoadthemeSetup {
	...
	function installplugin_function ()  {
		$this->james_register_required_plugins();	
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['plugin_slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( ' Activating.....', 'james' ),
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['plugin_slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( ' Updating......', 'james' ),
					'loading_url' => $loading_url
				);
				break;
			}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['plugin_slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( ' Installing ......','james'),
					'loading_url' => $loading_url
				);
				break;
			}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( ' Success', 'james' ),'loading_url' => $loading_url_success ) );
		}
